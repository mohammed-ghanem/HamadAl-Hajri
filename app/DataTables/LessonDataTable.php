...
class LessonDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ->eloquent($query)
            ->addColumn('checkbox', 'admin.lessons.btn.checkbox')
            ->addColumn('edit', 'admin.lessons.btn.edit')
            ->addColumn('delete', 'admin.lessons.btn.delete')
            ->addColumn('status',  function (Lesson $lesson) {

        return $lesson->status == 1 ? trans('admin/lessons/create.text_published') : trans('admin/lessons/create.text_hidden') ;

            })
            ->addColumn('category_id', function (Lesson $lesson) {
    ...
    {
        return Lesson::query()->with('category','videos');
    ...
    public function html()
    {
        return $this->builder()
                    ...
                    [
                        ...
                        'buttons'      => 
                     [
                        [ 'text'  => '<i class="fa fa-plus"></i> '.trans('admin/template/common.text_create_lesson'),'className'=>'addBtn',
                        "action"=>"function(){
